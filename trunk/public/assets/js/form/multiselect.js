...
(function($) {
   ...
   $.rgbmultiselector = function(input, prefs) {
	 ...
	 var selectWidth;
	 if($sSelect.attr("autoWidth")!=null){
		if($sSelect.attr("autoWidth")=="true"){
			sAutoWidth=true;
		}
		else{
			sAutoWidth=false;
	...
	if(sAutoWidth==true){
		selectWidth=parseInt($sSelect.outerWidth(),10);
	}
     else{
	 	selectWidth=120;
	 }
	 ...
	 var value
	 try {
		value=getItemId(optionsOneSelectedMatch,optionsOneSelectedMatchType);
	 ...
	 return;
	    ...
	    offset =$sInput.offset();
		 var top
	   if ($('#scrollContent').length>0){
	   		top =parseInt(offset.top+$("#scrollContent").scrollTop()-fixHeightTop,10);
	   }
	   else{
...
var fixHeightTop=0;
function getFixHeightTop(){
	$('#scrollContent').prevAll("div").each(function(){
		fixHeightTop=fixHeightTop+$(this).height();
		if($(this).css("marginBottom")!="auto"){
			fixHeightTop=fixHeightTop+parseInt($(this).css("marginBottom"));
		}
		if($(this).css("marginTop")!="auto"){
			fixHeightTop=fixHeightTop+parseInt($(this).css("marginTop"));
		}
	})
}

function multiSelRefresh(compId){
	var $curSel;
	if(typeof(compId)=="object"){
		var curSelId=$curSel.attr("id");
		$curSel=compId;
		$("#"+curSelId+"_rgbmultiselect").remove();
		$("#"+curSelId+"_rgbmultiselect_container").remove();
		$curSel.rgbmultiselect();
	}
	else{
		$curSel=$("#"+compId);
		$("#"+compId+"_rgbmultiselect").remove();
		$("#"+compId+"_rgbmultiselect_container").remove();
		$curSel.rgbmultiselect();
