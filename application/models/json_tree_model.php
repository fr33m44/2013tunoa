...
class Tree_Model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT " . implode(" , ", $this->fields) . " FROM " . $this->table . " WHERE " . $this->fields["id"] . " = " . (int) $id);
        return $query->num_rows() > 0 == null ? false : $query->row_array();
    }
    ...
    function _get_children($id, $recursive = false)
    {
        ...
        if ($recursive) {
            ...
            $query = $this->db->query("SELECT " . implode(" , ", $this->fields) . " FROM " . $this->table . " WHERE " . $this->fields["left"] . " >= " . (int) $node[$this->fields["left"]] . " AND " . $this->fields["right"] . " <= " . (int) $node[$this->fields["right"]] . " ORDER BY " . $this->fields["left"] . " ASC");
        } else {
            $query = $this->db->query("SELECT " . implode(" , ", $this->fields) . " FROM " . $this->table . " WHERE " . $this->fields["parent_id"] . " = " . (int) $id . " ORDER BY " . $this->fields["position"] . " ASC");
        }
        foreach ($query->result_array() as $row)
            $children[$row[$this->fields["id"]]] = $query->row_array();
        return $children;
    }
    ...
    function _get_path($id)
    {
        ...
        while ($this->db->nextr())
            $path[$row($this->fields["id"])] = $query->row_array();
        return $path;
    }
    ...
    function _move($id, $ref_id, $position = 0, $is_copy = false)
    {
        ...
        $ind = $this->db->insert_id($this->table);
        if ($is_copy)
    ...
    function _reconstruct()
    {
        ...
        while ($counter <= $maxcounter) {
            ...
            if ($this->db->nextr()) {
                ...
                $q = "INSERT INTO temp_stack (stack_top, " . $this->fields["id"] . ", " . $this->fields["left"] . ", " . $this->fields["right"] . ", " . $this->fields["level"] . ", " . $this->fields["parent_id"] . ", " . $this->fields["position"] . ") VALUES(" . ($currenttop + 1) . ", " . $tmp . ", " . $counter . ", NULL, " . $currenttop . ", " . $row("pid") . ", " . $row("lid") . ")";
                $this->db->query($q);
    ...
    function _analyze()
    {
        ...
        $report[] = ($row(0) != 1) ? "[FAIL]\tRoot node's left index is not 1." : "[OK]\tRoot node's left index is 1.";
        ...
        $this->db->nextr();
        $report[] = ($row(0) > 0) ? "[FAIL]\tMissing parents." : "[OK]\tNo missing parents.";
        ...
        $this->db->nextr();
        $report[] = ($row(0) > 0) ? "[FAIL]\tAdjacency and nested set do not match." : "[OK]\tNS and AJ match";
...
class Json_Tree_Model extends Tree_Model
{
    ...
    function set_data($data)
    {
        ...
        $s = "UPDATE " . $this->table . " SET " . $this->fields["id"] . " = " . (int) $data["id"];
    ...
    function search($data)
    {
        ...
        while ($this->db->nextr()) {
            $q .= " OR (" . $this->fields["left"] . " < " . (int) $row(0) . " AND " . $this->fields["right"] . " > " . (int) $row(1) . ") ";
        }
        $result = array();
        $this->db->query($q);
        while ($this->db->nextr()) {
