...
$(function(){
	if($(".nav_icon_v_item").length>0){
		var navIconVIdx=0;
		var navIconVIndex=jQuery.jCookie('navIconVIndex');
		if(navIconVIndex!=false){
			navIconVIdx=parseInt(navIconVIndex);
		}
		$(".nav_icon_v_item >a").filter(':eq('+navIconVIdx+')').addClass("current");
			$(".nav_icon_v_item >a").each(function(i){
			$(this).click(function(){
				$(".nav_icon_v_item >a").removeClass("current");
				$(this).addClass("current");
				jQuery.jCookie('navIconVIndex',i.toString());
			})
		})
	}
})