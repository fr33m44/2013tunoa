...
(function($) {
  ...
  $.cleditor = {
    ...
    buttons: {
      ...
      "center,居中对齐,justifycenter|" +
      "alignright,右对齐,justifyright|" +
      "justify,两端对齐,justifyfull|" +
      "undo,撤销,|" +
      "redo,重做,|" +
      "rule,插入水平线,inserthorizontalrule|" +
      "image,插入图像,图像,url|" +
  ...
  function buttonClick(e) {
      ...
      if (popupName) {
        ...
        if (popupName == "url") {
          ...
          if (buttonName == "link" && selectedText(editor) == "") {
            showMessage(editor, "插入链接之前要选中文字.", buttonDiv);
            return false;
          }
    ...
    // Check for button clicks in html mode
      ...
      showMessage(editor, " 按钮'" + button.title + 
      "' 在查看HTML时不可用", buttonDiv);
      return false;
    }
  ...
  function createPopup(popupName, options, popupTypeClass, popupContent, popupHover) {
    ...
    // URL
      ...
      $popup.html('输入URL:<br><input type=text value="http://" size=35><br><input type=button value="提交">');
  ...
  function execCommand(editor, command, value, useCSS, button) {
    ...
    if (command.toLowerCase() == "inserthtml" && $.browser.msie)
      ...
      if (!success)
        showMessage(editor, "您的浏览器不支持该项功能.", button);
    }
 ...
 $(document).ready(function() {
	$("textarea[class=rich]").each(function(){
		var editorWidth=500;
		var editorHeight=300;
		if($(this).attr("editorWidth")!=null){
			editorWidth=Number($(this).attr("editorWidth"));
		}
		if($(this).attr("editorHeight")!=null){
			editorHeight=Number($(this).attr("editorHeight"));
		}
		if($(this).attr("mode")=="full"){
			$(this).cleditor({
				width:editorWidth,
				height:editorHeight
			});
		}
		else if($(this).attr("mode")=="normal"||$(this).attr("mode")==null){
			$(this).cleditor({
					width:editorWidth,
					height:editorHeight,
					controls: "bold italic underline | size " +
		                    "style | color highlight removeformat | outdent " +
		                    "indent | alignleft center alignright | rule link unlink | html"
				});
		}
		else if($(this).attr("mode")=="min"){
			$(this).cleditor({
					width:editorWidth,
					height:editorHeight,
					controls: "bold italic underline | color highlight removeformat | alignleft center alignright | link unlink "
				});
		}
	})
  });
