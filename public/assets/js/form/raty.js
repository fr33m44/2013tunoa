...
(function($) {
	...
	$.fn.raty = function(settings) {
		...
		for (var i = 1; i <= options.number; i++) {
			...
			$this
			.append('<img width="16" id="' + containerId + '-' + i + '" src="' + path + starFile + '" altName="' + i + '" title="' + hint + '" class="' + containerId + '"/>')
		...
		if (!options.readOnly) {
			if (options.showCancel) {
				var cancel = '<img width="16" src="' + path + options.cancelOff + '" altName="x" title="' + options.cancelHint + '" class="button-cancel"/>'; 
			...
			.live('click', function() {
				$('input#' + containerId + '-score').val($(this).attr("altName"));
				if (onClick) { 
		          onClick($(this).attr("altName"));
		        }
			});
	...
	function liveClick() {
		...
		$('img.' + id).live('click', function() {
			$('input#' + id + '-score').val($(this).attr("altName"));
		});
	};
